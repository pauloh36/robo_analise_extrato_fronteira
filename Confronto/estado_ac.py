...
class Estado_ac:
    ...
    def logica_ac(self, arquivo_atual, caminho_arquivo, caminho_nfs_nao_efetivadas, filial_filtro, df_save):

        e = Estado_ac()
        u = utilidades.Utilidades()
        aba = aba_save.Aba_save()
        ...
        receita_1632 = (
            analiseStSefaz.loc[analiseStSefaz['Código Receita'] == 1632.0, ['Número nota', 'Valor icms sefaz']]
            .groupby(['Número nota', 'Valor icms sefaz'])
            .sum()
            )

        soma_cods_sefaz = [receita_1632]
        ...
        frame_fora_mes = frameOriginal.loc[(frameOriginal['Código Receita'] == 1632)]
