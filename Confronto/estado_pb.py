...
class Estado_pb:
    ...
    def logica_pb(self, arquivo_atual, caminho_arquivo, caminho_nfs_nao_efetivadas, filial_filtro, df_save):
        ...
        receita_1106 = (
            analiseStSefaz.loc[analiseStSefaz['Código Receita'] == 1106.0, ['Número nota', 'Valor icms sefaz']]
            .groupby(['Número nota', 'Valor icms sefaz'])
            .sum()
            )

        receita_1131 = (
            analiseStSefaz.loc[analiseStSefaz['Código Receita'] == 1131.0, ['Número nota', 'Valor icms sefaz']]
            .groupby(['Número nota', 'Valor icms sefaz'])
            .sum()
            )

        receita_9007 = (
            analiseStSefaz.loc[analiseStSefaz['Código Receita'] == 9007.0, ['Número nota', 'Valor icms sefaz']]
            .groupby(['Número nota', 'Valor icms sefaz'])
            .sum()
            )

        soma_cods_sefaz = [receita_1106, receita_1131, receita_9007]
        ...
        frame_fora_mes = frameOriginal.loc[(frameOriginal['Código Receita'] == 1106) |
                                           (frameOriginal['Código Receita'] == 1131) |
                                           (frameOriginal['Código Receita'] == 9007)
                                           ]
        ...
        # ----------- frame com as mercadorias de cada nota para criar a aba do save  ------------
